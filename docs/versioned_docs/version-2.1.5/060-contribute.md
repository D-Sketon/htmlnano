...
1. Create a ES6-file inside `lib/modules/` with a function that does some minification. For example you can check [`src/_modules/example.ts`](https://github.com/maltsev/htmlnano/blob/master/src/_modules/example.ts).

2. Add the module's name into one of those [presets](https://github.com/maltsev/htmlnano/tree/master/src/presets). You can choose either `ampSafe`, `max`, or `safe`.
